...
def test_create_ollama_provider_default():
    provider = create_ai_provider_from_config({})
    assert isinstance(provider, OllamaProvider)
    assert provider.model == "qwen2.5-coder:7b"


def test_create_ollama_provider_configured_model():
    provider = create_ai_provider_from_config({"ai_provider": "ollama", "ollama_model": "unit-test-model"})
